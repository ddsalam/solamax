...
Kontrol unit harus sesuai cakupan yang diminta sebelum menafsirkan hasil kosong. Periksa pointer/manifest untuk setiap tanggal kanari, bukan hanya respons HTTP. H1 tetap membutuhkan laporan EasyMax 12 September dan tidak dianggap lulus oleh backfill.

## Perintah Scheduler yang diserahkan ke Dion

Gunakan kanari 7 hari terlebih dahulu. Satu request bekerja pada satu unit; beberapa item dijalankan berurutan dengan lease global 1, deadline 18 menit, batas lease 04:45 dan publikasi sebelum 05:00 WIB. Payload batas 31 hari baru setelah kanari diterima. Retry Scheduler tetap 0; retry durable ada di database.

Variabel `SNAPSHOT_SECRET` berikut harus disiapkan lewat prosedur tepercaya yang sudah ada pada runbook pemicu 12 September. Blok ini tidak mengambil, mencetak, merotasi, atau mengubah binding secret. Jangan menjalankan dengan shell tracing. Output mutasi dibatasi `value(name)` agar respons resource tidak mencetak header.
...
# Enam job berikut hanya sesudah kapasitas/cut unit dan kanari disetujui Dion.
...
Sesudah Dion menerima hasil kanari, perintah berikut menaikkan rentang IB menjadi 31 hari sebelumnya. Default kode tetap 7 agar request lama tidak mendadak menyemai 31 hari. Nilai 0 berarti hari ini saja; batas keras 31 dan delapan item per request.
