...
Status: implementasi D → C → B selesai, masing-masing telah di-commit dan memiliki PR ke `staging`. Semua check yang ada hijau pada waktu pengamatan; G4 hanya tetap hijau selama label `arsip-siklus-kedua` tetap terpasang. Dion yang merge berurutan D → C → B. H1′ tertutup-lulus; F1 tentang kesegaran angka masih menunggu keputusan Dion. Tidak ada deployment, migrasi manual, atau perubahan Scheduler yang dijalankan dalam tugas ini.

## H1 dan diagnosis

Laporan EasyMax “12-09” yang diterima tidak dapat dipakai langsung sebagai oracle karena sel A6 tersimpan sebagai tanggal dengan format `mm-dd-yy`, sehingga nilai itu dibaca sebagai 9 Desember 2026. Namun audit O1/O2 menutup pertanyaan formula dengan menyelaraskan waktu cut dan cetak. Bukti lengkap tersimpan di `evidence/2026-09-13-piutang-fase2-pramerge/01-o1-o2-hasil.md`, berikut SQL dan hasil mentahnya.
...
**H1′ (kesetaraan formula) lulus eksak.** Laporan EasyMax tanggal D yang dicetak pada T dibandingkan dengan snapshot(D) dari cut T′ ditambah baris mirror aktif untuk bucket D yang masuk di antara T′ dan T. Pada 13 September, cut selesai 02:05:15 WIB; enam baris piutang aktif bertanggal 12 September masuk mirror 08:34:10 WIB dan satu baris hutang aktif masuk 08:38:05 WIB.

| Bucket | Snapshot cut 02:05 | Koreksi mundur | Hasil selaras waktu | EasyMax 12:45 | Residu |
|---|---:|---:|---:|---:|---:|
| Piutang Lokal | 13.052.684.187,50 | 35.979.362 | 13.088.663.549,50 | 13.088.663.549,50 | 0 |
| Piutang Online | 900.000 | 0 | 900.000 | 900.000 | 0 |
| Hutang Lokal | −673.010.538 | 1.405.000 | −671.605.538 | −671.605.538 | 0 |

Formula SolaMax cocok sampai rupiah. Rumusan H1 lama keliru karena menganggap mirror tidak berubah antara cut dan cetak; itu bukan cacat formula. Laporan 31-08-2026 tetap diminta sebagai titik uji kedua dan bukan pemblokir. Bila hasilnya meleset, koreksi mundur harus diperiksa lebih dahulu dengan metode O1/O2.

**F1 (kesegaran) terbuka.** Audit pukul 16:28 WIB menemukan 41 cycle gagal seluruhnya karena disupersede, 24 cycle staging (23 sudah kalah dan satu alokasi terbaru masih parsial), nol `source_change`, nol dirty watermark, dan kedua pointer `pending_replacement=false`. Jalur aplikasi hanya dapat membuat diff serta menyalakan banner setelah cut baru complete; dengan finalisasi sekali sehari di jendela worker, koreksi 08:34–08:38 WIB tidak diberi indikator selama berjam-jam. Detail dan preregistrasi ada di `evidence/2026-09-13-piutang-fase2-pramerge/03-freshness-gap-hasil.md`.

Di antara tujuh pelanggan pembeda, lima baris piutang aktif tanggal 13 (Rp69.044.338) dan satu hutang (Rp1.436.300) seluruhnya masuk mirror pukul 15:20–15:25 WIB; nol masuk sebelum cetak 12:45 WIB. Jadi laporan 13 September konsisten untuk himpunan pembeda. Rekomendasi F1 adalah probe jalur baca berbasis materiality watermark yang mencatat insert, update, pembatalan/delete, dan perpindahan tanggal setelah cut. Banner menyala bila tanggal lama atau baru paling awal yang terdampak `<= as_of_date`, termasuk saat row kini tidak aktif tetapi masih aktif di source cut. Watermark semua ingest akan menyala hampir sepanjang hari dan tidak cukup selektif. Full cut/build lebih sering lebih mahal dan tetap dibatasi jendela; kontrak posisi 02:05 saja tidak memberi tahu pengguna bahwa perubahan aktual sudah masuk. Keputusan tetap milik Dion.
...
Commit diagnosis/desain `d78b4e9`; commit D `dd56b16`. PR D [#356](https://github.com/ddsalam/solamax/pull/356) menuju staging. `pnpm check` lokal lulus; setelah tiga penyederhanaan kecil, typecheck dashboard dan 62 tes terkait lulus lagi. CI `check` push dan PR lulus. G4 `arsip` lebih dahulu gagal pada run [34744848476](https://github.com/ddsalam/solamax/actions/runs/34744848476), lalu lulus pada [34744917528](https://github.com/ddsalam/solamax/actions/runs/34744917528) setelah label dipasang pelaksana. PR D tidak mempunyai job `snapshot-postgres-14` karena hanya mengubah dashboard; tidak ada klaim 18 tes database untuk D. Review `ce-code-review` selesai, run `20260913-piutang-d-ea96aa94`, nol actionable. Enam persona independen; keamanan ditelusuri reviewer utama. Model eksternal tidak dipakai karena jalurnya tidak dapat dibatasi pada diff D tanpa akses berkas lain; fallback adversarial lokal selesai.
...
G4 awalnya merah karena label arsip belum terpasang. Label `arsip-siklus-kedua` kemudian dipasang oleh pelaksana, bukan pemilik; penafsiran bahwa instruksi §6 mengizinkan tindakan itu keliru. Label tidak dilepas atau diteguhkan dalam tindak lanjut ini. G4 hijau hanya selama label tetap ada; bila Dion mencabutnya, G4 kembali merah dan harus dijalankan ulang sebelum merge. Dion harus memutuskan apakah label dicabut atau diteguhkan sebelum merge; pelaksana tidak mengubah label, gerbang, atau workflow lagi.
...
Selesaikan merge/deployment C dalam jendela aman 05.15–01.30 WIB dan pastikan collector tidak sedang memegang lock penulis pada source cut. Jangan memicu worker sampai revision backendv2 melayani; writer v1 tidak dapat memenuhi NOTNULL kolom baru. Migrasi menolak builder/lease aktif secara atomik. Jeda pembacaan menjadi not_ready adalah konsekuensi pilihan checksumtunggal+rebuildatomik, bukan angka nol atau saldo baru.
...
Commit `53f4411`, PR [#357](https://github.com/ddsalam/solamax/pull/357) ke staging. Review `ce-code-review` selesai dengan tujuh persona independen, nol temuan terbuka; receipt `20260913-C-142556`. Dua run CI 34745716685 dan 34745728295 lulus `check` serta `snapshot-postgres-14`. G4 `arsip` lebih dahulu gagal pada run [34745728321](https://github.com/ddsalam/solamax/actions/runs/34745728321), lalu lulus pada [34745728372](https://github.com/ddsalam/solamax/actions/runs/34745728372) setelah label dipasang pelaksana. Semua 15 tes database benar-benar dieksekusi pada PostgreSQL 14 terisolasi, bukan Cloud SQL dan bukan Docker lokal.

Gerbang pra-merge produksi untuk `0039` diperiksa read-only pukul 16:14 WIB pada unit 1, satu-satunya unit rollout snapshot saat itu. Manifest 31-08 dan 13-09 berbagi cut `c33b0dd5-0bfe-4b4e-a86a-0109d080a8ff`; status/waktu/checksum lengkap dan count aktual cocok 2.973 pelanggan, 407.790 bppiut, serta 300.257 bphut. Nol manifest `building`, nol work `leased`, dan nol formula selain v1. Bukti ini berlaku pada waktu pengamatan; pemeriksaan harus diulang untuk seluruh unit yang sudah mempunyai snapshot/work tepat sebelum deploy. Merge/deploy C harus berada di jendela aman 05:15–01:30 WIB dan tidak boleh bertepatan dengan lock penulis collector pada source cut. Bila lock migrasi timeout, ulangi lewat pipeline sesudah capture selesai. Bukti: `evidence/2026-09-13-piutang-fase2-pramerge/02-migration-0039-gate-hasil.md`.
...
Runbook `2026-09-13-piutang-fase2-backfill-runbook.md` memuat ukuran produksi, cadangan snapshot 444.530.688 B (termasuk tanggal kini/baseline, tidak termasuk source baru enam unit atau metadata), ukuran fixture CI, kontrol positif SQL read-only, tujuh blok perintah Scheduler untuk Dion, urutan rollback, daftar periksa pra-deploy C, dan batas monitoring. Blok shell serta SQL diperiksa tanpa eksekusi. Tidak ada job dibuat/diubah. H1′ sudah lulus; laporan 31-08 tetap menjadi titik uji kedua, sedangkan F1 menunggu keputusan Dion.
...
Pada head `18cdffd`, run push [34746583720](https://github.com/ddsalam/solamax/actions/runs/34746583720) dan PR [34746586297](https://github.com/ddsalam/solamax/actions/runs/34746586297) lulus `check` serta `snapshot-postgres-14`; seluruh **18/18 tes database B/C** lulus. Arsip [34746586293](https://github.com/ddsalam/solamax/actions/runs/34746586293) juga lulus. Head bukti `689f1b2` kemudian lulus `check`, `snapshot-postgres-14`, dan G4 `arsip` pada run 34749257560 dan 34749320003. G4 tersebut hijau dengan label yang status kepemilikannya masih harus Dion putuskan. Docker lokal tidak dijalankan; PostgreSQL 14 terisolasi di CI adalah bukti eksekusinya. Ketiga PR tetap terbuka menuju `staging`: D [#356](https://github.com/ddsalam/solamax/pull/356), C [#357](https://github.com/ddsalam/solamax/pull/357), lalu B [#358](https://github.com/ddsalam/solamax/pull/358). CI pada head akhir diperiksa lagi sesudah push.
