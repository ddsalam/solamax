...
- **RLS unit-scoped (lapis-DB, hard gate tenant ke-2)** ✅ **LIVE di IB (2026-07-07)** — `0016_rls_unit_scope` di `solamax-pg`: **26 tabel unit-scoped RLS (ENABLE+FORCE)**, policy `unit_scope` baca GUC `app.unit_ids` (di-set `qScoped()` dashboard + `set_config` ingest). Backend `solamax-ingest-staging-00024-fzr` + dashboard `solamax-dashboard-staging-00036-v4w` keduanya **`rls-aware=1` @100%**; instance di-bump ke **db-g1-small** (max_connections=50). `0017_audit_log` (append-only) live. Terverifikasi: fail-closed (no-context=0 baris), cross-unit write ditolak WITH CHECK, **14/14 domain ingest menulis di bawah RLS**, 0 error 4xx/5xx. DDL dijalankan sebagai role pemilik `ingest`. Runbook [`apps/backend/RLS-CUTOVER-RUNBOOK.md`](apps/backend/RLS-CUTOVER-RUNBOOK.md); ledger `session-notes/rls-rehearsal/LEDGER.md`. Rollback instan `apps/backend/scripts/rls-rollback.sql` (as ingest) tersedia bila perlu. ⚠️ **Batas yang dijaga lapis ini (dikoreksi 2026-08-07):** RLS melindungi dari query yang **LUPA men-scope** (fail-closed → nol baris); ia **TIDAK** melindungi dari pemanggil yang menyerahkan **scope yang SALAH** — sebab `qScoped()` menurunkan GUC dari argumen pertamanya, jadi scope yang dilebarkan melebarkan RLS juga. Yang menjaga terhadap scope-salah adalah brand `ScopedUnitId` (lapis TIPE), seluruhnya. Jangan pakai kata "backstop" untuk menalar keamanan di sini — rincian di [`0016_rls_unit_scope/CORRECTION.md`](apps/backend/prisma/migrations/0016_rls_unit_scope/CORRECTION.md) (berkas migrasinya sengaja TIDAK disunting: checksum `prisma migrate deploy`).
...
- **Saldo Piutang/Hutang (blok RECAP Laporan Operasional)** ✅ **diperbaiki & dikunci 2026-08-06** — ⛔ **komentar "formula terkunci … EKSAK 27-Jun" di [`0013_recap_saldo_tables/migration.sql:5-8`](apps/backend/prisma/migrations/0013_recap_saldo_tables/migration.sql) SUDAH TIDAK BERLAKU** (salah di tiga hal: batas `<`, `Online = SJENIS 3`, dan klaim EKSAK yang benar hanya terhadap **laporan yang salah**). Berkas migrasinya **sengaja tidak disunting** — checksum `prisma migrate deploy` terbukti identik byte, menyuntingnya menghentikan CD; koreksi di `CORRECTION.md` sedirektori. **Sumber kebenaran = `getSaldoPelanggan`** di [`apps/dashboard/src/lib/queries.ts`](apps/dashboard/src/lib/queries.ts). Aturan: Piutang Lokal = SJENIS {1,5} **DAN** kode tanpa titik · Piutang Online = kode **bertitik**, **tanpa** filter SJENIS · Hutang = seluruh `bphut`, dinegatifkan; semua `sbatal=0`, disajikan pada **dua batas** (awal hari `<` & akhir hari `<=`). Diskriminator Lokal/Online = **format kode, bukan SJENIS** (dua sumbu — menutup D3). 🔴 **ORACLE yang sah HANYA "DAFTAR SALDO HUTANG PIUTANG"**, jangan "Laporan Penjualan Harian" (definisinya beda: saldo awal vs akhir hari) — mencampurnya adalah akar hantu "IB kurang 19,7 miliar" yang tak pernah ada. Terbukti **24 sel** di 2 unit (28 Oktober 9/9, IB 15/15) + 1.640 titik rekonsiliasi per-pelanggan. `SALDO_EXPECTED` di `probe.ts` **dipensiunkan**. Bukti: [`session-notes/2026-08-05-saldo-hutang-piutang-28oktober.md`](session-notes/2026-08-05-saldo-hutang-piutang-28oktober.md).

- **Artefak floating-point kolom `numeric`** ✅ **sebab ditemukan & ditambal 2026-08-06** — Prisma merender parameter JS `number` ke **16 angka penting** (bukan shortest-roundtrip), jadi `73867616.46` mendarat sbg `73867616.45999999`. Bukan `num()` di agent (JSON shortest-roundtrip ✓), bukan Postgres (`::float8::numeric` bersih ✓). Sapuan mirror: **±243.800 sel, 7 unit** (`sales_detail` 159k · `opname` 45k · `edc` 20k · `pelanggan_sale` 19k) — **tak material**: deviasi maks 1 sel 0,00001, deviasi TOTAL semuanya **0,0000746**; jalur baca kebal (`::float8`) dan tak ada perbandingan `<> 0` yang tersentuh (min |nvolselisih| ≠ 0 = 0,01 di tiap unit). Fix: `NUMERIC_COLUMNS` di [`sql.ts`](apps/backend/src/ingest/sql.ts) → param **teks** + `::numeric`; dijaga unit test (uji merahnya sudah dijalankan). Baris historis: domain `mode:"full"` sembuh sendiri; sisanya lewat [`scripts/backfill-numeric-artifacts.sql`](apps/backend/scripts/backfill-numeric-artifacts.sql) (`trim_scale(round(k,4))` — **jangan** bulatkan ke 2: `tera.liter` punya 10 nilai sah 3/4 desimal) — ⛔ **belum dijalankan, butuh gerbang owner + HARUS setelah fix live**. Bukti: [`session-notes/2026-08-06-artefak-float-numeric-ingest.md`](session-notes/2026-08-06-artefak-float-numeric-ingest.md).

- **Ketaatan Administrasi (indikator pengisian Rincian oleh pengawas)** — keputusan yang mengikat, alasannya, dan batas yang diketahui ada di **[`apps/dashboard/KETAATAN-ADMINISTRASI.md`](apps/dashboard/KETAATAN-ADMINISTRASI.md)**. Baca itu SEBELUM menyentuh `compliance.ts` / `rekon.ts` / lantai adopsi / ketiga gerbangnya. Ringkas: toleransi Rp 1.000 = kuantum slip setoran (aritmetika, bukan gaya) · lantai adopsi BEKU di config (bukan `min()` hidup) · tiga gerbang yang masing-masing menjaga hal BERBEDA · SATU pembuat vonis (`adminStatus`) dengan PDF & kartu setoran sebagai hilir · batas: atestasi per-hari 4,1%, `sqlcheck` tak menjaga `main`, **lubang F&G laten** (memperbaiki `I` saja agar cocok dengan `H` yang sudah salah menghijaukan hari yang komponennya milik hari lain — dijaga INSTRUKSI, bukan aturan). Ekor C/D: pertanyaan **DITUTUP sebagai keputusan** 2026-08-09 — paparan ≤ ~20 menit karena shift terakhir tersinkron pagi hari berikutnya, jadi `shifts < SHIFT_TARGET` sudah menahan semalaman; gerbang keempat sengaja TIDAK dibangun.

- **Keuangan Harian (model akuntansi penuh per unit — CF/IS/BS + gerbang tutup hari)** — keputusan yang mengikat, alasannya, batas yang diketahui, dan **gerbang masuk K1 (§9)** ada di **[`apps/dashboard/KEUANGAN-HARIAN.md`](apps/dashboard/KEUANGAN-HARIAN.md)**. Baca itu SEBELUM menyentuh apa pun yang berbau keuangan. **Fase K0 (investigasi) SELESAI 2026-08-10; K1 belum dibuka — belum ada tabel/migrasi/kode.**
...
1. **Postgres RLS unit-scoped (lapis-DB)** — ✅ **SATISFIED — live di IB 2026-07-07.** Unit-scoped RLS (`0016_rls_unit_scope`) live di `solamax-pg`: 26 tabel ENABLE+FORCE, jaring kedua **terhadap query yang LUPA men-scope** — bukan terhadap scope yang SALAH (lihat batas di §status). **HARD GATE sebelum tenant nyata ke-2 → TERPENUHI.** Detail lihat status di atas + runbook/ledger.
