...
d("Bakau⊥IB scope isolation (data nyata, fixture-free)", () => {
  ...
  it("direksi PT → melihat KEDUA IB + Bakau", (ctx) => {
    ...
    const a = allowed(ctxAll("direksi", pt!));
  ...
  it("pengawas[Bakau] → HANYA Bakau (bukan IB)", (ctx) => {
    if (!bakau) return ctx.skip();
    const a = allowed(ctxUnits("pengawas", pt!, [bakau!.unit_id]));
  ...
  it("pengawas[IB] → HANYA IB (bukan Bakau)", (ctx) => {
    if (!bakau) return ctx.skip();
    const a = allowed(ctxUnits("pengawas", pt!, [ib!.unit_id]));
  ...
  it("404 lintas-unit: pengawas Bakau minta IB (dan sebaliknya) → tak terlihat", (ctx) => {
    ...
    expect(
      unitVisible(
        ctxUnits("pengawas", pt!, [bakau!.unit_id]),
        { unit_id: ib!.unit_id, tenant_id: ib!.tenant_id },
      ),
    ).toBe(false);
    expect(
      unitVisible(
        ctxUnits("pengawas", pt!, [ib!.unit_id]),
