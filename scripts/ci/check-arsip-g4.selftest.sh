...
periksa "arsip berubah + label terpasang → LOLOS" 0 "dinyatakan terpenuhi" \
  BERUBAH=session-notes/x.md LABELS=dokumentasi,arsip-siklus-kedua LABEL_ADA=true
...
periksa "label mirip ('arsip-siklus-kedua-lah') tidak melepas" 1 "belum dinyatakan bertahan" \
  BERUBAH=session-notes/x.md LABELS=arsip-siklus-kedua-lah LABEL_ADA=true

if [ "$gagal" != 0 ]; then
  echo "self-test G4: GAGAL"; exit 1
fi
echo "self-test G4: 5 keadaan lulus (termasuk jalur hijau)."
